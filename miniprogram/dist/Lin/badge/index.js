    ...
    methods: {
        ...
        finalCount: function finalCount() {
            switch (this.data.countType) {
              ...
              case "limit":
                this.setData({
                    finalCount: parseInt(this.data.count) >= 999 ? parseInt(this.data.count) >= 9999 ? Math.floor(this.data.count / 1e4 * 100) / 100 + "w" : Math.floor(this.data.count / 1e3 * 100) / 100 + "k" : this.data.count
                });
                break;
