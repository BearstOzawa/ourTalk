Page({
    ...
    submitTemplateMessageForm: function submitTemplateMessageForm(e) {
        var _this = this;
        this.setData({
            templateMessageResult: ""
        });
    ...
    onGetWXACode: function onGetWXACode() {
        var _this2 = this;
        this.setData({
            wxacodeSrc: "",
            wxacodeResult: "",
        ...
        // 此处为演示，将使用 localStorage 缓存，正常开发中文件 ID 应存在数据库中
                var fileID = wx.getStorageSync("wxacodeCloudID");
        if (fileID) {
            // 有云文件 ID 缓存，直接使用该 ID
    ...
    clearWXACodeCache: function clearWXACodeCache() {
